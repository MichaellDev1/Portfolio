...
function IconSkill ({ res, theme }) {
  return (
    <div key={res.skill} className='relative inline-block px-3 rounded-md py-2 item-skills' style={{ backgroundColor: theme.cardSkill }}>
      ...
      <div className='w-14 h-14 mb-1 text-3xl flex justify-center items-center icon-skill rounded-lg' style={{ backgroundColor: theme.iconContent }}><span style={{ zIndex: '3', color: theme.text }}>{res.icon}</span></div>
      ...
      <span className='text-lg font-semibold' style={{ color: theme.text }}>{res.skill}</span>
    </div>
  )
}
