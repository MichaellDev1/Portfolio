...
function IconSkill ({ res, theme }) {
  return (
    <div key={res.skill} className='relative inline-block px-3 py-2 item-skills ai-showcase' style={{ backgroundColor: theme.cardSoftSkill, borderRadius: '4px' }}>
      {res.inProcess ? <span className='w-[6px] h-[6px] right-2 bg-lime-500 absolute rounded-full inProcess' /> : null}
      <div className='w-14 h-14 mb-2 text-3xl flex justify-center items-center icon-skill rounded-md' style={{ backgroundColor: theme.iconContent }}><span style={{ zIndex: '3', color: theme.text }}>{res.icon}</span></div>
